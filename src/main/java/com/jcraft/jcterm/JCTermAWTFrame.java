...
//  public void keyPressed(KeyEvent event){}
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        /*
         * else if(action.equals("X11 Forwarding...")){ String display=null;
         * display=JOptionPane.showInputDialog(this, "XDisplay name (hostname:0)",
         * (xhost==null)? "": (xhost+":"+xport)); try{ if(display!=null){
         * xhost=display.substring(0, display.indexOf(':'));
         * xport=Integer.parseInt(display.substring(display.indexOf(':')+1));
         * xforwarding=true; } } catch(Exception ee){ xforwarding=false; xhost=null; } }
         */
        /*
         * else if((action.equals("AntiAliasing"))){ setAntiAliasing(!antialiasing); }
         */
        else if (action.equals("About...")) {
            MessageDialog msg = new MessageDialog("About...", JCTermAWT.COPYRIGHT);
        } else if (action.equals("Compression...")) {
            InputDialog dialog = new InputDialog("Compression level(0-9)", new Integer(compression).toString(), false);
            ...
            InputDialog dialog = new InputDialog("Line Space", new Integer(line_space).toString(), false);
...
//				      "Establish the connection before this setting.");
        ...
        /*
         * else if (action.equals("passwd")){ ui.passwd=textf.getText();
         * dialog.setVisible(false); return; } else if (action.equals("passphrase")){
         * ui.passphrase=textf.getText(); dialog.setVisible(false); return; } else if
         * (action.equals("SOCKS5... return")){ String proxy=textf.getText();
         * if(proxy.length()==0){ setProxySOCKS5(null, 0); dialog.setVisible(false);
         * return; } try{ String foo=proxy.substring(0, proxy.indexOf(':')); int
         * bar=Integer.parseInt(proxy.substring(proxy.indexOf(':')+1)); if(foo!=null){
         * setProxySOCKS5(foo, bar); dialog.setVisible(false); } } catch(Exception ee){
         * } }
         * 
         * else if ((action.equals("Local Port... return")) ||
         * (action.equals("Remote Port... return"))){ String foo=textf.getText(); int
         * port1=Integer.parseInt(foo.substring(0, foo.indexOf(':')));
         * foo=foo.substring(foo.indexOf(':')+1); String host=foo.substring(0,
         * foo.indexOf(':')); int
         * port2=Integer.parseInt(foo.substring(foo.indexOf(':')+1));
         * 
         * if(action.equals("Local Port...")){ setPortForwardingL(port1, host, port2); }
         * else{ setPortForwardingR(port1, host, port2); } dialog.setVisible(false); }
         * else if (action.equals("HTTP... return")){ String proxy=textf.getText();
         * if(proxy.length()==0){ setProxyHttp(null, 0); dialog.setVisible(false);
         * return; } try{ String foo=proxy.substring(0, proxy.indexOf(':')); int
         * bar=Integer.parseInt(proxy.substring(proxy.indexOf(':')+1)); if(foo!=null){
         * setProxyHttp(foo, bar); dialog.setVisible(false); } } catch(Exception ee){ }
         * } else if (action.equals("Open SHELL Session... return") ||
         * action.equals("Open SFTP Session... return") ){ try{ String
         * _host=textf.getText(); String _user=_host.substring(0, _host.indexOf('@'));
         * _host=_host.substring(_host.indexOf('@')+1); if(_host==null ||
         * _host.length()==0){ return; } user=_user; host=_host;
         * dialog.setVisible(false); openSession(); } catch(Exception ee){ } }
         */
    }
...
//    mi.setActionCommand("Remote Port...");
        m.add(mi);
        /*
         * mi=new MenuItem("X11 Forwarding..."); mi.addActionListener(this);
         * mi.setActionCommand("X11 Forwarding..."); m.add(mi); mb.add(m);
         */

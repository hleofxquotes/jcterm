...
public class JCTermAWT extends Panel implements KeyListener, /* Runnable, */ Term {
    ...
    /*
     * private String xhost="127.0.0.1"; private int xport=0; private boolean
     * xforwarding=false; private String user=System.getProperty("user.name");
     * private String host="127.0.0.1";
     * 
     * private String proxy_http_host=null; private int proxy_http_port=0;
     * 
     * private String proxy_socks5_host=null; private int proxy_socks5_port=0;
     */
...
//      descent+=line_space;
    ...
    void initGraphics() {
        ...
        /*
         * background=createImage(char_width, char_height); { Graphics
         * foog=background.getGraphics(); foog.setColor(getBackGround());
         * foog.fillRect(0, 0, char_width, char_height); foog.dispose(); }
         */
     ...
     * MyUserInfo ui=null;
     * 
     * public void run(){ while(thread!=null){ try{ try{ } catch(Exception e){
     * //System.out.println(e); break; } requestFocus(); emulator=new
     * EmulatorVT100(this, in); emulator.reset(); emulator.start(); }
     * catch(Exception e){ //e.printStackTrace(); } break; }
     * 
     * thread=null;
     * 
     * if(splash!=null) splash.draw(img, getTermWidth(), getTermHeight()); else
     * clear();
     * 
     * redraw(0, 0, getTermWidth(), getTermHeight()); }
     */
...
//System.out.println(e);
        ...
        if (id == KeyEvent.KEY_PRESSED) {
            ...
            /* keyReleased(e); */ } else if (id == KeyEvent.KEY_TYPED) {
            keyTyped(e);
            /* keyTyped(e); */ }
        e.consume(); // ??
    }
...
//    term_area.repaint(x, y, width, height);
    ...
    public void clear_area(int x1, int y1, int x2, int y2) {
        /*
         * for(int i=y1; i<y2; i+=char_height){ for(int j=x1; j<x2; j+=char_width){
...
//    graphics.setColor(getForeGround());
    ...
    /*
     * public class MyUserInfo implements UserInfo{ public boolean
     * promptYesNo(String str){ // System.out.println(str); PromptDialog dialog=new
     * PromptDialog("Warning",str); while(true){
     * try{Thread.sleep(1000);}catch(Exception e){} if(!dialog.isVisible()){ break;
     * } } return dialog.result; }
     * 
     * String passwd=null; String passphrase=null;
     * 
     * public String getPassword(){ return passwd; } public String getPassphrase(){
     * return passphrase; }
     * 
     * public boolean promptPassword(String message){ InputDialog dialog=new
     * InputDialog(message, "", true); passwd=dialog.getText(); return passwd!=null;
     * } public boolean promptPassphrase(String message){ InputDialog dialog=new
     * InputDialog(message, "", true); passphrase=dialog.getText(); return
     * passphrase!=null; } public void showMessage(String message){ MessageDialog
...
//  public void keyPressed(KeyEvent event){}
    ...
    /*
     * public void setProxyHttp(String host, int port){ proxy_http_host=host;
     * proxy_http_port=port; if(proxy_http_host!=null && proxy_http_port!=0){
     * proxy=new ProxyHTTP(proxy_http_host, proxy_http_port); } else{ proxy=null; }
     * } public String getProxyHttpHost(){return proxy_http_host;} public int
     * getProxyHttpPort(){return proxy_http_port;}
     * 
     * public void setProxySOCKS5(String host, int port){ proxy_socks5_host=host;
     * proxy_socks5_port=port; if(proxy_socks5_host!=null && proxy_socks5_port!=0){
     * proxy=new ProxySOCKS5(proxy_socks5_host, proxy_socks5_port); } else{
     * proxy=null; } } public String getProxySOCKS5Host(){return proxy_socks5_host;}
     * public int getProxySOCKS5Port(){return proxy_socks5_port;} public void
     * setXHost(String xhost){this.xhost=xhost;} public void setXPort(int
     * xport){this.xport=xport;} public void setXForwarding(boolean
     * foo){this.xforwarding=foo;}
     */
    ...
    /*
     * public static void main(String[] arg){ JCTermAWT term=new JCTermAWT(); Frame
     * frame=new Frame("JCTermAWT");
     * 
     * frame.addWindowListener(new WindowAdapter(){ public void
     * windowClosing(WindowEvent e){System.exit(0);} });
     * 
     * MenuBar mb=term.getMenuBar(); frame.setMenuBar(mb);
     * 
     * frame.setSize(term.getTermWidth(), term.getTermHeight()); frame.add("Center",
     * term);
     * 
     * frame.pack(); term.setVisible(true); frame.setVisible(true);
     * 
     * term.initGraphics();
     * 
     * frame.setResizable(true); { Insets insets=frame.getInsets(); int
     * foo=term.getTermWidth(); int bar=term.getTermHeight();
     * foo+=(insets.left+insets.right); bar+=(insets.top+insets.bottom);
     * frame.setSize(foo, bar); } frame.setResizable(false);
     * 
     * term.setFrame(frame); } public void quit(){ thread=null; if(session!=null){
